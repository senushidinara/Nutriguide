...
const App: React.FC = () => {
  ...
  const handleAddFoodWithQuantity = (food: Omit<FoodItem, 'quantity' | 'unit'>, quantity: string, unit: string) => {
    if (currentMeal.length >= 8) {
      alert("The Energy Core is reaching maximum capacity! Please simulate with up to 8 items.");
      return;
    };
  ...
  const handleSimulate = useCallback(async () => {
    if (!userProfile || currentMeal.length === 0) {
      setError("Please add food to the Energy Core before simulating.");
      return;
    }
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      setChatHistory([{ role: 'model', text: "I've reviewed your simulation. What's on your mind?" }]);
    } catch (e) {
      console.error(e);
      setError("The AI failed to simulate the meal. This could be due to a configuration issue or an unusual food combination. Please try again.");
    } finally {
      setIsLoading(false);
    }
